...
public class ClientService : IClientService
{
    ...
    public async Task<Client> GetByDniAsync(string dni)
    {
        ...
        if (client == null) throw new NotFoundException("Cliente con ese DNI no encontrado.");
        return client;
    }
    ...
    public async Task<IEnumerable<Client>> GetAllAsync(int? enterpriseId = null)
    {
        ...
        if (enterprise == null) throw new NotFoundException($"Empresa con ID {enterpriseId.Value} no encontrada.");
    ...
    public async Task<Client> GetByIdAsync(int id)
    {
        ...
        if (client == null) throw new NotFoundException("Cliente no encontrado.");
        return client;
    }
    ...
    public async Task<Client> CreateAsync(Client entity, int enterpriseId)
    {
        ...
        if (enterprise == null) throw new NotFoundException($"Empresa con ID {enterpriseId} no encontrada.");
        ...
        if (existingClient != null) throw new BadRequestException("Ya existe un cliente con la misma razón social.");
    ...
    public async Task<bool> AssignClientToEnterpriseAsync(int clientId, int enterpriseId)
    {
        ...
        if (exists) throw new BadRequestException("El cliente ya está asignado a esta empresa.");
    ...
    public async Task<Client> UpdateAsync(int id, Client entity, int enterpriseId)
    {
        ...
        if (relationship == null)
            throw new BadRequestException(
                $"El cliente con ID {id} no está asociado a la empresa con ID {enterpriseId}.");
        ...
        if (entity.phone != client.phone) await ValidatePhoneAsync(entity.phone);
    ...
    public async Task<bool> RemoveClientFromEnterpriseAsync(int clientId, int enterpriseId)
    {
        ...
        if (enterpriseClient == null) throw new NotFoundException("El cliente no está asignado a esta empresa.");
    ...
    public async Task<int> GetTotalClientsActivesAsync(int enterpriseId)
    {
        ...
        var status =
            await _context.EnterpriseClients.FirstOrDefaultAsync(
                s => s.status == 'A' && s.enterprise_id == enterpriseId);
        if (status == null) throw new NotFoundException("No hay clientes activos en esta empresa.");
    ...
    private async Task ValidatePhoneAsync(string phone)
    {
        ...
        if (existingClient != null) throw new BadRequestException("Ya existe un cliente con el mismo teléfono.");
    }
    ...
    private async Task ValidateDniClientAsync(string dni)
    {
        ...
        if (existingClient != null) throw new BadRequestException("Ya existe un cliente con el mismo DNI.");
    }
    ...
    public async Task<ResponseDto> DesactiveClientAsync(int clientId, int enterpriseId)
    {
        ...
        try
        {
            ...
            if (client == null)
            {
                ...
                response.DisplayMessage = "Cliente no encontrado.";
                return response;
            }
            var enterpriseClient = await _context.EnterpriseClients
                .FirstOrDefaultAsync(ec => ec.client_id == clientId && ec.enterprise_id == enterpriseId);
            ...
            {
                response.Success = false;
                response.DisplayMessage = "El cliente no está asignado a esta empresa.";
                return response;
            }

            enterpriseClient.status = 'I';
            client.status = 'I';
            
            await _context.SaveChangesAsync();
            response.DisplayMessage = "Cliente desactivado exitosamente.";
        }
        catch (Exception ex)
        {
            response.Success = false;
            response.DisplayMessage = "Error al desactivar el cliente: " + ex.Message;
        }
    ...
    public async Task<ResponseDto> ActiveClientAsync(int clientId, int enterpriseId)
    {
        var stopwatch = Stopwatch.StartNew();
        var response = new ResponseDto();
        ...
        try
        {
            ...
            enterpriseClient.status = 'A';
            client.status = 'A';
            
            await _context.SaveChangesAsync();
            response.DisplayMessage = "Cliente activado exitosamente.";
        }
        catch (Exception ex)
        {
            response.Success = false;
            response.DisplayMessage = "Error al activar el cliente: " + ex.Message;
        }
